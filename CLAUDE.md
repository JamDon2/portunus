...
bun tauri build -- --features ocr               # build with OCR (requires libtesseract-dev + libleptonica-dev)
...
**Scoring** — Composite score = category base + fuzzy score (nucleo-matcher) + recency bonus + frecency bonus. Base scores by kind: clipboard 5M, timer 4M, calc 3M, app 2M, file 1M, folder 0. Results with `nucleo_score < MIN_NUCLEO_SCORE` are filtered out.
...
**Content index** (`content_index.rs`) — SQLite full-text search over file contents. Optional OCR via Tesseract (`--features ocr`). Triggered by `trigger_full_reindex` command or watcher. Progress events emitted as `content-index-progress { indexed, total }`.
...
**IPC** (`ipc.rs`) — Unix socket at `$XDG_RUNTIME_DIR/portunus.sock` handles `--show`, `--clipboard`, `--reindex`, `--reload-config` CLI flags from a second instance.
...
- `libpdfium.so` — PDF preview via `pdfium-render`. Install on Arch: `yay -S pdfium-bin`. Loaded at runtime via `Pdfium::bind_to_system_library()`.
- `dict` (dictd) — Dictionary lookups. Silently disabled if not found at runtime.
- `cliphist` + `wl-clipboard` — Clipboard history provider.
- `poppler` — PDF content indexing (not preview).
